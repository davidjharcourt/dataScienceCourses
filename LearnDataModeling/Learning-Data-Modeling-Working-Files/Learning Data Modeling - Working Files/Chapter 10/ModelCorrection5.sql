-- The SQL script now has the following revision.
-- Correction 5 -- Change
--    1 AccountCustomer to many Address
-- to
--    0..1 AccountCustomer to many Address
-- This revision has the same effect as running
--    ALTER TABLE Address ALTER COLUMN accountCustomerID bigint NULL;
--
-- Also removed constraint on Customer that phoneNumber or emailAddress is NOT NULL.
-- This revision has the same effect as running
--    ALTER TABLE Customer DROP CONSTRAINT CK__Customer__108B795B;
...
ALTER TABLE [Customer]
	 WITH CHECK ADD  CHECK  NOT FOR REPLICATION ( customerType IN ('AdhocCustomer', 'AccountCustomer') )
go
...
CREATE NONCLUSTERED INDEX [XIF1AccountCustomer] ON [AccountCustomer]
( 
	[preferredAddressID]  ASC
)
go

CREATE NONCLUSTERED INDEX [XIF2AccountCustomer] ON [AccountCustomer]
( 
...
ALTER TABLE [CreditCard]
	 WITH CHECK ADD  CHECK  NOT FOR REPLICATION ( cardType IN ('VISA', 'Mastercard', 'American Express', 'Discover') )
go
...
( 
	...
	[pickupDatetime]     datetime  NULL 
		CHECK  ( pickupDatetime > getDate() OR pickupDatetime IS NULL ),
...
ALTER TABLE [MenuItem]
	 WITH CHECK ADD  CHECK  NOT FOR REPLICATION ( menuItemType IN ('beverage', 'salad', 'soup', 'sandwich') )
go

ALTER TABLE [MenuItem]
	 WITH CHECK ADD  CHECK  NOT FOR REPLICATION ( menuItemSize IN ('small',  'medium',  'large',  'x-large') )
go
...
( 
	...
	[orderItemName]      varchar(50)  NULL ,
	 PRIMARY KEY  CLUSTERED ([orderItemID] ASC),
		...
		CHECK  ( (orderItemName IS NULL AND menuItemID IS NOT NULL) OR
...
CREATE VIEW [AdhocCustComplete]([adhocCustomerID],[customerName],[phoneNumber],[emailAddress])
AS
...
CREATE VIEW [AccountCustComplete]([accountCustomerID],[customerName],[phoneNumber],[emailAddress],[accountNumber],[encryptedPassword],[preferredAddressID],[preferredCreditCardID])
AS
