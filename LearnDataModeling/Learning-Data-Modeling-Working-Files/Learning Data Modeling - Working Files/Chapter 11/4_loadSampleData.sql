...
/* The following code does not work in Access.
...
FROM SandwichShop;     -- The table name here is just a placeholder to satisfy SQL syntax.
   The previous code does not work in Access */

INSERT INTO OrderItem (quantity, orderItemAmount, menuItemID, custOrderID)
SELECT 1
     , 0
     , (SELECT menuItemID FROM MenuItem WHERE menuItemName = 'custom sandwich' AND menuItemSize = 'small')
     , (SELECT MAX (custOrderID) FROM CustOrder)
FROM SandwichShop;

UPDATE OrderItem OI, ListPrice LP, MenuItem MI, Discount D
SET OI.orderItemAmount = LP.listPriceAmount
WHERE MI.menuItemName = 'custom sandwich' AND MI.menuItemSize = 'small' AND D.discountName = 'none' AND
      MI.menuItemID = LP.menuItemID AND LP.discountID = D.discountID AND
      OI.orderItemID = (SELECT MAX (orderItemID) from OrderItem);


/* The following code does not work in Access
INSERT INTO OrderItem_IngredientChoice (orderItemID, ingredientChoiceID)
SELECT
       (SELECT MAX (orderItemID) FROM OrderItem)
...
SELECT
       (SELECT MAX (orderItemID) FROM OrderItem)
     , (SELECT ingredientChoiceID
        FROM IngredientChoice IC, ItemIngredient II, MenuItem MI
        WHERE menuItemName = 'custom sandwich' AND itemIngredientName = 'bread' AND ingredientChoiceName = 'white' AND
              IC.itemIngredientID = II.itemIngredientID AND II.menuItemID = MI.menuItemID)
FROM SandwichShop;
...
SELECT
     ...
     , (SELECT ingredientChoiceID
        FROM IngredientChoice IC, ItemIngredient II, MenuItem MI
        WHERE menuItemName = 'custom sandwich' AND itemIngredientName = 'cheese' AND ingredientChoiceName = 'swiss' AND
              IC.itemIngredientID = II.itemIngredientID AND II.menuItemID = MI.menuItemID)
FROM SandwichShop;
...
SELECT
     ...
     , (SELECT ingredientChoiceID
        FROM IngredientChoice IC, ItemIngredient II, MenuItem MI
        WHERE menuItemName = 'custom sandwich' AND itemIngredientName = 'meat' AND ingredientChoiceName = 'ham' AND
              IC.itemIngredientID = II.itemIngredientID AND II.menuItemID = MI.menuItemID)
FROM SandwichShop;
...
INSERT INTO OrderItem (quantity, orderItemAmount, menuItemID, custOrderID)
SELECT 1
     , (SELECT listPriceAmount FROM ListPrice LP, MenuItem MI, Discount D
        WHERE menuItemName = 'garden salad' and discountName = 'none' AND MI.menuItemID = LP.menuItemID AND LP.discountID = D.discountID)
     , (SELECT menuItemID FROM MenuItem WHERE menuItemName = 'garden salad')
     , (SELECT MAX (custOrderID) FROM CustOrder)
FROM SandwichShop;     -- The table name here is just a placeholder to satisfy SQL syntax.
...
SELECT 1
     , (SELECT listPriceAmount FROM ListPrice LP, MenuItem MI, Discount D
        WHERE menuItemName = 'tea' and discountName = 'none' AND MI.menuItemID = LP.menuItemID AND LP.discountID = D.discountID)
     , (SELECT menuItemID FROM MenuItem WHERE menuItemName = 'tea')
     , (SELECT MAX (custOrderID) FROM CustOrder)
FROM SandwichShop;
