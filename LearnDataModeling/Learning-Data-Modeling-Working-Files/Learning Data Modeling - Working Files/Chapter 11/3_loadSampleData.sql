...
/* The following code does not work in MS-Access
UPDATE AccountCustomer
SET preferredAddressID = (SELECT addressID FROM Address WHERE addressData = '71113 Walnut Street  Someplace, New Mexico  87500 USA')
WHERE accountCustomerID IN
 (SELECT customerID FROM Customer WHERE customerName = 'Moe Stooge');
   The previous code does not work in MS-Access */

UPDATE AccountCustomer AC, Address A, Customer C
SET AC.preferredAddressID = A.addressID
WHERE C.customerName = 'Moe Stooge' AND AC.accountCustomerID = C.customerID AND
      A.addressData = '71113 Walnut Street  Someplace, New Mexico  87500 USA';
...
UPDATE AccountCustomer AC, CreditCard CC, Customer C
SET AC.preferredCreditCardID = CC.creditCardID
WHERE C.customerName = 'John Smith' AND AC.accountCustomerID = C.customerID AND
      CC.cardNumber = '6666-333222-11111';
